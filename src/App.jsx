...
// IMPORTANT: ตรงนี้ต้องแก้เป็นค่าของคุณเอง (ดูวิธีด้านล่างสุดของคู่มือ)
...
// --- Helper: PDF Generator (Image Based for Thai Support) ---
const generatePDFWithImages = async (items, projectName) => {
  ...
  const cardWidth = (pageWidth - (margin * 3)) / cols; // margin left, middle, right
  const cardHeight = (pageHeight - 40 - (margin * 4)) / rows; // 40 is header space
  ...
  // Loop through items and capture images
  for (let i = 0; i < items.length; i++) {
    ...
    const y = 30 + (row * (cardHeight + margin)); // 30 is start Y after title

    // Find DOM element
    const elementId = `card-${item.id}`;
    const element = document.getElementById(elementId);
    
    if (element) {
      // Hide delete button before capture
      ...
      try {
        ...
        const pdfImgHeight = (imgProps.height * cardWidth) / imgProps.width;
        
        // Center vertically in the cell if it's shorter than max height
        let finalY = y;
        let finalH = pdfImgHeight;
        
        if (pdfImgHeight > cardHeight) {
           finalH = cardHeight;
        }

        doc.addImage(imgData, 'JPEG', x, finalY, cardWidth, finalH);
      } catch (err) {
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      // สำหรับ Environment ปกติ ใช้ signInAnonymously อย่างเดียว
      // หรือถ้ามีการส่ง token มาค่อยใช้ CustomToken
